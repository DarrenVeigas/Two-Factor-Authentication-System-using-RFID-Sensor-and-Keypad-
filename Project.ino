...
const int Code2MifareCard1 = '5';              // Determine here the second digit of your 6 digit PIN
...
const int Code2MifareCard2 = '0';              // Determine here the second digit of your 6 digit PIN
const int Code3MifareCard2 = '*';              // Determine here the third digit of your 6 digit PIN
const int Code4MifareCard2 = '#';              // Determine here the fourth digit of your 6 digit PIN
...
//----------------------------------------------Code for detect the first Mifare Card in the Database. This is a UID number with 4 pairs of digits. Example: 69 7C 01 9C------------------------------------
   ...
   
     {
      ...
      MifareCard1 = 1;                                   // This Mifare Card is known as Mifare Card 2 in the Database, so set variable MifareCard2 to value 1.                      // Make a beep for the accepted Mifare Card             
      delay (150);                                       //                       //
...
//-----------------------------------------------Code for detect the second Mifare Card in the Database. This is a UID number with 4 pairs of digits. Example: EB 70 C0 BC----------------------------------------------
     if                                                    
     (mfrc522.uid.uidByte[0] == 0x73   &&                 // Fill in the first digits of the UID number of your Mifare card
      mfrc522.uid.uidByte[1] == 0xF4   &&                 // Fill in the seconds digits of the UID number of your Mifare card              
      mfrc522.uid.uidByte[2] == 0x3B   &&                 // Fill in the third digits of the UID number of your Mifare card
      mfrc522.uid.uidByte[3] == 0xC5)                     // Fill in the fourth digits of the UID number of your Mifare card
...
if (Code4Correct == 1 && RightCard == 1)                    // If the PinCode is the correct and you did enter this within 7 seconds
  {
...
if ((Code4Correct == 0) && (PinCodeCounter >= 4) && (RightCard == 1))       // If you offered a valid Mifare Card and the Pincode is not correct and you already entered 6 digitkeys 
  {  
...
if ((WrongPinCode == 1) || (millis()- Timer > 7000 && RightCard == 1))      // If you offered a valid Mifare Card and  you entered a wrong PinCode or the timer (7 seconds) expires
  {                                            // Make a long beep and set the Red LED HIGH for the wrong code or timer expiry
...
if ((RightCard == 1) &&                                                    // If you offered a valid Mifare Card and any DigitKey on the keypad is pressed
    ...
    {                               
      PinCodeCounter++;                                                       // PinCodeCounter value +1 for every press on any Digitkey on the keypad                                            // Make a short beep for a DigitKey press on the keypad
      delay (50);                                                             //                                            //
    }
